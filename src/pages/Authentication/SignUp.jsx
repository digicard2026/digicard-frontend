...
const Signup = () => {
  ...
  // Continue navigation after email verification
  const handleContinue = () => {
    if (franchiseContext) {
      navigate("/signin/franchise");
    } else if (location.state?.selectedPlan) {
  ...
  const handleResendEmail = async () => {
    try {
      ...
      else alert(data.error || "Failed to resend email.");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white p-10 rounded-2xl shadow-lg text-center w-full max-w-md">
        ...
        {/* Professional Blue Heading */}
        <h2 className="text-3xl font-semibold text-blue-600 mb-4">
          Verify Your Email
        </h2>

        {/* Normal text matching signup font style */}
        ...
        {/* Neutral professional button instead of yellow */}
        <button
          onClick={handleResendEmail}
          className="w-full bg-gray-200 text-gray-700 py-3 rounded-lg font-medium hover:bg-gray-300 transition-all mb-4"
        >
          Resend Verification Email
        </button>

        {/* Primary action blue button (same tone as Signup page) */}
        <button
          onClick={handleContinue}
