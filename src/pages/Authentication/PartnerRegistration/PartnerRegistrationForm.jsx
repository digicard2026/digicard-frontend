...
import * as Yup from 'yup';
import { useLocation } from "react-router-dom";
import FormFields from './PartnerFormFields';
const API_URL = import.meta.env.VITE_API_URL;
...
const PartnerRegistrationForm = () => {
  ...
  // ✅ Load user context from localStorage (for after signup)
  useEffect(() => {
    const loadUserContext = () => {
      const userId = localStorage.getItem('user_id');
      const userEmail = localStorage.getItem('user_email');

      console.log('🔍 Loading user context from localStorage:', { userId, userEmail });
      ...
      if (userId && userEmail) {
        ...
        setFormData(prev => ({ 
          ...prev, 
          email: userEmail || prev.email,
          personalEmail: userEmail || prev.personalEmail
        }));
      }
    };
      ...
      <div className="text-center pt-8 px-8">
        ...
        <div className="mb-4 p-3 bg-blue-50 rounded-lg">
          <p className="text-blue-700 text-sm">
            Account created for: <span className="font-semibold">{userContext.userEmail}</span>
          </p>
        </div>
      </div>
