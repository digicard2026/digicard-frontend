...
// FIXED: Accept both data and cardData props for compatibility
const ModernCard = ({ cardData = {}, data = {}, plan = 'BusinessPremium' }) => {
  console.log('📱 ModernCard received:', { cardData, data, plan });
  
  // Use either cardData or data (for compatibility with different CardRenderer versions)
  ...
  // Build profileData from safeData
  const profileData = {
    // Personal Info
    prefix: safeData?.prefix || "",
    firstName: safeData?.firstName || safeData?.first_name || "",
    lastName: safeData?.lastName || safeData?.last_name || "",
    suffix: safeData?.suffix || "",
    name: `${safeData?.prefix || ""} ${safeData?.firstName || safeData?.first_name || ""} ${safeData?.lastName || safeData?.last_name || ""}`.trim() || "Your Name",
    tagline: safeData?.tagline || safeData?.headline || "",
    
    // Professional Info
    jobTitle: safeData?.jobTitle || safeData?.job_title || safeData?.position || "",
    companyName: safeData?.companyName || safeData?.company_name || safeData?.company || "",
    department: safeData?.department || "",
    foundedName: safeData?.foundedName || "",
    organization: safeData?.organization || "",
    
    // Contact Info
    email: safeData?.email || safeData?.primaryEmail || "",
    emails: safeData?.emails || [{ address: safeData?.email || "", label: "primary", isPrimary: true }],
    phones: safeData?.phones || [{ number: safeData?.phone || "", isPrimary: true }],
    websites: safeData?.websites || [{ url: safeData?.website || "" }],
    addresses: safeData?.addresses || [{ 
      fullAddress: safeData?.address || "",
      isPrimary: true,
      googleMapsLink: "https://maps.google.com"
    }],
    virtualNumber: safeData?.virtualNumber || { number: "", isEnabled: false },
    
    // Profile Content
    profileVideo: safeData?.profileVideo || null,
    productVideo: safeData?.productVideo || null,
    videos: safeData?.videos || [],
    aboutText: safeData?.aboutText || safeData?.about || safeData?.description || "",
    bio: safeData?.bio || safeData?.about || "",
    servicesProducts: safeData?.servicesProducts || safeData?.services || "",
    brandLabel: safeData?.brandLabel || "",
    catalogPDF: safeData?.catalogPDF || "",
    catalog: safeData?.catalog || safeData?.catalogPDF || "",
    
    // Business Hours
    businessHours: safeData?.businessHours || [],
    
    // Social & Media
    socialLinks: safeData?.socialLinks || safeData?.social_links || [],
    profilePhoto: safeData?.profilePhoto || safeData?.profile_pic || safeData?.avatar || safeData?.image || "https://images.unsplash.com/photo-1494790108755-2616b786d4d1?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80&fit=facearea&facepad=3",
    companyLogo: safeData?.companyLogo || safeData?.company_logo || "",
    ...
    clientList: safeData?.clientList || safeData?.clients || [],
    gallery: safeData?.gallery || [],
    downloads: safeData?.downloads || [],
    interactiveElements: safeData?.interactiveElements || [],

    // Additional Fields
    customFields: safeData?.customFields || [],
    dynamicQRCode: safeData?.dynamicQRCode || null,
    shareableUrl: safeData?.shareableUrl || safeData?.shareUrl || "",
    individualProductDisplay: safeData?.individualProductDisplay || false,
    businessCardInstagram: safeData?.businessCardInstagram || "",
    ...
    // Chat features
    chatAssistant: safeData?.chatAssistant || { isEnabled: false, welcomeMessage: 'Hello! How can I help you today?' },
    liveChat: safeData?.liveChat || { isEnabled: false, platform: 'whatsapp' },
    
    // Design & Layout
    design: safeData?.design || 'default',
    cardLayout: safeData?.cardLayout || 'standard',
    logoSize: safeData?.logoSize || 'medium',
    productRangeDisplay: safeData?.productRangeDisplay || 'grid',
    
    // Contact Management Toggles
    enableOneTapCall: safeData?.enableOneTapCall !== undefined ? safeData.enableOneTapCall : true,
    enableWhatsApp: safeData?.enableWhatsApp !== undefined ? safeData.enableWhatsApp : true,
    enableEmail: safeData?.enableEmail !== undefined ? safeData.enableEmail : true,
  };
  ...
  const renderShareButton = () => {
    ...
    return (
      <div className="mb-12">
        ...
        <button
          onClick={handleShare}
          className="mx-auto flex items-center justify-center px-6 py-3 rounded-full border-2 border-purple-500 text-purple-600 hover:bg-purple-50 transition-all duration-300 transform hover:scale-105 active:scale-95 font-medium"
        >
          <FaShare className="w-5 h-5 mr-2" />
          Share My Card
        </button>
  ...
  const renderBrandLabel = () => {
    ...
    return (
      <div className="text-center mb-12">
        ...
        <div className="relative rounded-2xl p-5 mx-auto max-w-[320px]">
          <div className="absolute inset-0 rounded-2xl border-2 border-purple-300 bg-gradient-to-br from-purple-50 to-pink-50"></div>
          <div className="relative z-10">
  ...
  // FIXED: Using proper className instead of style attribute
  return (
    <>
      {/* Fixed: Changed style tag to not cause React warnings */}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600&family=Montserrat:wght@400;500;600;700&display=swap');
      ...
      {/* Mobile View */}
      <div className="min-h-screen md:hidden bg-white modern-card-font">
        <div className="bg-white overflow-hidden relative w-full h-full">
          <div className="overflow-y-auto h-full">
            ...
            >
              ...
              {/* Profile Photo */}
              <div className="absolute -bottom-16 left-1/2 transform -translate-x-1/2 z-10">
                <div className="rounded-full border-4 border-white overflow-hidden shadow-2xl relative w-32 h-32 bg-white">
                  <img 
                    src={profileData.profilePhoto}
                    alt={profileData.name}
                    className="w-full h-full object-cover relative z-10"
            ...
            <div className="pt-20 pb-8 px-4">
              {/* Name and Designation */}
              <div className="text-center mb-6">
                <h1 className="text-2xl font-semibold text-black mb-2 modern-card-heading">
                  {profileData.name}
                </h1>
                {profileData.jobTitle && (
                  ...
                  <div className="flex justify-center space-x-6 mb-12">
                    ...
                    {primaryPhone && primaryPhone.number && profileData.enableWhatsApp && (
                      <button
                        ...
                        className="flex flex-col items-center justify-center"
                      >
                        <div className="rounded-full flex items-center justify-center text-white mb-1 w-12 h-12 bg-gradient-to-br from-green-500 to-green-600 shadow-md">
                    ...
                    {primaryPhone && primaryPhone.number && profileData.enableOneTapCall && (
                      <button
                        ...
                        className="flex flex-col items-center justify-center"
                      >
                        <div className="rounded-full flex items-center justify-center text-white mb-1 w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 shadow-md">
                    ...
                    {profileData.email && profileData.enableEmail && (
                      <button
                        ...
                        className="flex flex-col items-center justify-center"
                      >
                        <div className="rounded-full flex items-center justify-center text-white mb-1 w-12 h-12 bg-gradient-to-br from-red-500 to-red-600 shadow-md">
              ...
              {/* ABOUT Section */}
              {(profileData.bio || profileData.aboutText) && (
                <div className="mb-12">
                  <h2 className="mb-6 text-center text-xl font-semibold text-black modern-card-heading">
                    ABOUT
                  </h2>
              ...
              {/* Tagline */}
              {currentPlan.showCheckUsOut && profileData.tagline && (
                <div className="relative rounded-full w-full max-w-[300px] h-12 px-4 py-2 flex items-center justify-center mt-0 mx-auto mb-8 border-2 border-black">
                  <div className="relative text-center z-10 px-2">
                    <FaCrown className="w-4 h-4 text-yellow-500 mx-auto mb-1" />
                    <p className="text-black font-semibold text-xs tracking-wide">
                      {profileData.tagline}
                  ...
                  {profileData.socialLinks
                    ...
                    .map((social, index) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="rounded-full flex items-center justify-center text-white hover:shadow-xl transition-all duration-300 w-12 h-12"
                        style={{
                          background: social.platform === 'linkedin' 
                            ? 'linear-gradient(135deg, #0077b5 0%, #005582 100%)'
                            : social.platform === 'twitter'
      ...
      {/* Desktop View */}
      <div className="min-h-screen hidden md:flex items-center justify-center p-6 overflow-auto bg-gradient-to-br from-blue-400 to-purple-600 modern-card-font">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden relative w-full max-w-[420px] min-h-[600px] max-h-[85vh]">
          <div className="overflow-y-auto h-full max-h-[85vh]">
            {/* Header */}
            <div className="h-60 relative bg-gradient-to-r from-purple-400 to-pink-500">
              {/* Company Logo */}
              {(currentPlan.showCheckUsOut && (profileData.companyLogo || profileData.companyName)) && (
                <div className="absolute top-4 left-0 right-0 text-center">
                  <div className="flex flex-col items-center justify-center">
            ...
            <div className="pt-20 pb-10 px-8">
              ...
              {(profileData.bio || profileData.aboutText) && (
                <div className="mb-16">
                  <h2 className="mb-10 text-center text-xl font-semibold text-black modern-card-heading">
                    ABOUT
                  </h2>
