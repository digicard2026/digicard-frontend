...
const LightCard = ({ cardData = {} }) => {
  ...
  useEffect(() => {
    ...
    const videoInterval = setInterval(() => {
      if (isFieldAllowed(profileData.cardType, 'profileVideo') && profileData.videos?.length > 0) {
  ...
  // Build complete profileData from cardData with ALL fields including new ones
  const profileData = {
    // Personal Info
    prefix: cardData?.prefix || "",
    firstName: cardData?.firstName || "",
    ...
    const allTabs = [
      ...
      { 
        ...
        allowed: isFieldAllowed(currentPlan, 'testimonials') || 
                ...
                isFieldAllowed(currentPlan, 'downloads') ||
                isFieldAllowed(currentPlan, 'interactiveElements') ||
                isFieldAllowed(currentPlan, 'videos') ||
                isFieldAllowed(currentPlan, 'catalogPDF') ||
  ...
  const renderBrandLabel = () => {
    if (!isFieldAllowed(currentPlan, 'brandLabel') || !profileData.brandLabel) return null;
    ...
    return (
      <div className="mb-4">
        ...
        <div className="relative rounded-xl p-4 bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-300/30">
          <p className="casper-font text-center text-sm font-medium">{profileData.brandLabel}</p>
        </div>
      </div>
    );
  };
  ...
  const renderServicesProductsOverview = () => {
    if (!isFieldAllowed(currentPlan, 'servicesProducts') || !profileData.servicesProducts) return null;
        ...
        <div className="hours-grid">
          {hours.map((hour, index) => (
            <div key={index} className="text-center p-2 bg-white/5 rounded-lg">
              <p className="rumaila-font text-xs">{hour.split(':')[0]}</p>
              <p className="casper-font text-xs mt-1">{hour.split(':')[1]}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderVirtualNumber = () => {
    if (!profileData.virtualNumber) return null;
    ...
    return (
      <div className="mb-4">
        ...
        >
          <span className="casper-font">{profileData.virtualNumber}</span>
        </button>
      </div>
    );
  };
  ...
  const renderCatalogPDF = () => {
    if (!isFieldAllowed(currentPlan, 'catalog') && !profileData.catalogPDF) return null;
    
    return (
      <div className="mb-4">
        <div className="flex items-center justify-center mb-2">
          <FaFileInvoice className="w-5 h-5 text-red-400 mr-2" />
          <h3 className="rumaila-font text-lg">Catalog PDF</h3>
        </div>
        <button
        ...
        >
          ...
          <span className="casper-font">Download Catalog</span>
        </button>
      </div>
    );
  };
  ...
  const renderVideosSlider = () => {
    if (!profileData.videos || profileData.videos.length === 0) return null;
            ...
            <div className="relative rounded-lg overflow-hidden h-40">
              ...
              {getCurrentVideoItem().title && (
                <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-2">
                  <p className="casper-font text-xs text-center">{getCurrentVideoItem().title}</p>
                </div>
              )}
            </div>
          )}
  ...
  // NFC Card Component (different from NFC Settings)
  const renderNFCCard = () => {
    if (!profileData.nfcCard) return null;
  ...
  const renderEnhancedClientList = () => {
    if (!isFieldAllowed(currentPlan, 'clientList') || profileData.clientList.length === 0) return null;
        ...
        <div className="client-grid">
          {displayClients.map((client, index) => (
            <div key={index} className="bg-white/5 rounded-lg p-2 text-center">
              <p className="casper-font text-xs truncate">{client}</p>
            </div>
          ))}
        </div>
  ...
  const ProfileHeader = () => (
    <div className="pb-4 pt-6 no-bg-border">
      ...
      <div className="relative flex flex-col items-center justify-center px-4">
        ...
        {/* Tagline Box - Only show if allowed by plan */}
        {isFieldAllowed(currentPlan, 'titleLine') && profileData.tagline && (
  ...
  const MoreTab = () => {
    ...
    const showQRCode = isFieldAllowed(currentPlan, 'dynamicQRCode') && profileData.dynamicQRCode?.targetUrl;
    const showNFC = isFieldAllowed(currentPlan, 'nfcSettings') && profileData.nfcSettings?.isEnabled;
    const showNFCCard = profileData.nfcCard;

    const hasContent = showTestimonials || showClientList || showDownloads || showInteractive || 
                      showCustomFields || showQRCode || showNFC || showNFCCard;
