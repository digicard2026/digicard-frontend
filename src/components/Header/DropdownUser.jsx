...
import { getCookie,deleteCookie } from '../../utility/cookies';
import { getUserById ,getUserProfile} from '../../service/userApi';
...
const DropdownUser = () => {
  ...
  useEffect(() => {
   ...
   try {
     ...
     console.log('proffiledata',profileData)
    //  const formattedFilePath = profileData.data.filePath.replace(/\\/g, '\\');
    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs" >{uin}</span>
        </span>

       <div className="h-11 w-11 rounded-full overflow-hidden flex items-center justify-center bg-slate-500 text-white text-xl font-bold">
  {imageUrl ? (
    <img
      src={
